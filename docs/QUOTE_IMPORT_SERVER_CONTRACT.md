...
> Contrat serveur implémenté dans `0009_quote_import_server_contract.sql`. La base distante n’a pas été modifiée.
...
L’option D est implémentée : une table versionnée pour le payload validé et une table relationnelle enfant pour les issues structurées. Les JSONB historiques restent des traces d’entrée et ne deviennent jamais automatiquement une preuve.
...
La fonction `validate_quote_import_row` reçoit l’identifiant de ligne et les valeurs opérationnelles autorisées nécessaires à la projection ; elle dérive opération, organisation et site depuis les tables persistées. Elle écrit payload et issues dans une transaction, calcule le statut, fige la version/hash et audite le résultat.
